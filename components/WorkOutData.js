  ...
  const deleteWorkOut = async (index) => {
    ...
    Alert.alert(
      ...
      `Haluatko varmasti poistaa treenin: ${
        workoutDataToDelete.workoutName
      } ${formatTimestamp(workoutDataToDelete.workoutId)} ?`, // Viesti
      [
        {
          text: "Peruuta",
          style: "cancel",
        },
        {
          ...
          onPress: async () => {
            try {
              ...
              console.log(
                `Workout ${workoutDataToDelete.workoutId} poistettu onnistuneesti.`
              );
              setUpdateContent((prevData) => prevData + 1);
            } catch (error) {
        ...
        <SwiperFlatList
          ...
          renderItem={({ item, index }) => (
            <View
              key={index}
              style={[
                styles({ colors, spacing }).workoutBox,
                {
                  justifyContent: biggerFavourite ? "space-between" : "center",
                },
              ]}
            >
              <View style={styles({ colors, spacing }).workOutDetails}>
                ...
                <View style={styles({ colors, spacing }).centerContent}>
                  <View style={styles({ colors, spacing }).textContainer}>
                    <Text
                      style={styles({ colors, spacing }).workoutBoxMainText}
                    >
                      {item.workoutName ? item.workoutName : "Treeni pvm:"}
                    </Text>
                    <Text
                      style={styles({ colors, spacing }).workoutBoxMainTextDate}
                    >
                  ...
                  <View style={styles({ colors, spacing }).iconContainer}>
                    ...
                    <TouchableOpacity
                      onPress={() =>
                        addWorkoutToFirebase(item.movements, item.workoutName)
                      }
                    >
                      <Icon
                        name={"heart-outline"}
                ...
                <View style={styles({ colors, spacing }).workoutContent}>
                  {item.movements.map((movement, idx) => (
                    <Text
                      ...
                      key={idx}
                    >
                      {movement.movementName}: {movement.sets.length} x{" "}
                      {movement.sets[0].reps}
                    </Text>
                  ))}
